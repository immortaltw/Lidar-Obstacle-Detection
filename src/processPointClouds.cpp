...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SeparateClouds(pcl::PointIndices::Ptr inliers, typename pcl::PointCloud<PointT>::Ptr cloud) 
{
    ...
    typename pcl::PointCloud<PointT>::Ptr cloud_c (new pcl::PointCloud<PointT>);

    for (int i: inliers->indices) cloud_p->points.push_back(cloud->points[i]); 
...
std::pair<typename pcl::PointCloud<PointT>::Ptr, typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::SegmentPlane(typename pcl::PointCloud<PointT>::Ptr cloud, int maxIterations, float distanceThreshold)
{
    ...
    if (inliers.size() == 0) {
        std::cout << "unable to separate" << std::endl;
    }

	pcl::PointIndices::Ptr cloudInliers (new pcl::PointIndices ());

	for(int index = 0; index < cloud->points.size(); index++)
	{
		// pcl::PointXYZI point = cloud->points[index];
		if(inliers.count(index))
			cloudInliers->indices.push_back(index);
	}
...
std::vector<typename pcl::PointCloud<PointT>::Ptr> ProcessPointClouds<PointT>::Clustering(typename pcl::PointCloud<PointT>::Ptr cloud, float clusterTolerance, int minSize, int maxSize)
{
	...
	for(int index = 0; index < cloud->points.size(); index++) {
        PointT pt_ = cloud->points[index];
        std::vector<float> ptv;
        ptv.push_back(pt_.x);
        ptv.push_back(pt_.y);
        ptv.push_back(pt_.z);
        points_.push_back(ptv);
        tree->insert(ptv, index);
    }

    // Perform euclidean clustering to group detected obstacles
    std::vector<std::vector<int>> clusters_ = euclideanCluster(points_, tree, clusterTolerance, minSize, maxSize);
